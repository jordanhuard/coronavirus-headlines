...
parser_opt.add_argument('--input_length',type=int, default=64)
...
def str_to_tensor(ss,input_length=None):
    ...
    if args.model == 'bert':
        for i,line in enumerate(ss):
            ...
            #print('tokens=',tokens)
        #generate 1-hot encodings of lines
...
def explain(line,filename,explain_type):
    ...
    if explain_type == 'char':
        for i in range(len(line)):
            ...
            new_tensor[i+1,0] = torch.zeros([len(vocabulary)])
            new_output,new_nextchars = model(new_tensor)
            new_probs = softmax(new_output)
            scores[i] = torch.norm(probs - new_probs)**2
    elif explain_type == 'word':
        count = 1
        word_lengths = [len(word) for word in line.split()]
        for word_length in word_lengths:
            new_tensor = input_tensor.clone()
            new_tensor[count:count+word_length,0] = torch.zeros(word_length, len(vocabulary))
            new_output,new_nextchars = model(new_tensor)
            new_probs = softmax(new_output)
            probs_norm = torch.norm(probs - new_probs)**2
            for i in range(count, count+word_length):
                scores[i-1] = probs_norm
...
if args.embed:
    ...
    if args.log_dir is None:
        raise ValueError('must give --log_dir to visualize an embedding')

    print('i know i am embedding')
    ...
    print('ok i supposedly embedded')
